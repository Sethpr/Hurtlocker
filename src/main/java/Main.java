...
public class Main {
    static final String[] pats = {"(?i)(?<=name:milk;price[:;])[0-9].[0-9][0-9](?=;)", "(?i)(?<=name:bread;price[:;])[0-9].[0-9][0-9](?=;)", "(?i)(?<=name:c[0o][0o]kies;price[:;])[0-9].[0-9][0-9](?=;)", "(?i)(?<=name:apples;price[:;])[0-9].[0-9][0-9](?=;)"};
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i = pats.length-1; i >= 0; i--) {
            process(Pattern.compile(pats[i]).matcher(output).results().map((p) -> p.group()).collect(Collectors.toList()),name[i], sb);
        }
